...
export default function ContactForm() {
    ...
    return (
        <form className={`max-w-[400px] mx-auto p-3 flex flex-col gap-5 border-2 border-${isDark ? 'zinc-300' : 'zinc-600'} text-black rounded-md`} onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <Button type='submit' text='Відправити' styles={`${buttonStyle} ${isDark ? 'bg-slate-400 hover:bg-slate-500' : 'bg-slate-100 hover:bg-slate-200'} transition-all sm:text-xl`} />
                        </form>
    );
}
